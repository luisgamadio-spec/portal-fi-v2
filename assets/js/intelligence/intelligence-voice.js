...
(function () {
  ...
  function bridgeToPortalIntelligence(message, callId, opts) {
    ...
    return window.NX_AUTH.getAccessToken().then(function (token) {
      ...
      // LATENCY-1 -- correlates this Voice diagnostic entry with the
      // exact same trace id (correlation_id, already generated by
      // sendRealText -- same shared adapter Text uses) the server-side
      // [bai-timing]-shaped stage_ms breakdown for THIS SAME call is
      // keyed by, without duplicating any of that breakdown here (it
      // already reaches this same result._devTiming object a few lines
      // below). Absent (not empty string) when _devTiming itself is
      // absent (fixture/error-before-fetch paths), same discipline as
      // every other optional field on this diagnostic event.
      diagPush('tool_call', { ms: Date.now() - t0, ok: !result.error, turn: turnIndex, trace_id: (result._devTiming && result._devTiming.correlation_id) || null });
