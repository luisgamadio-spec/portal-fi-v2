#!/usr/bin/env python3
"""
Gates 34-35 -- Dashbi (Análise Geral do Grupo) Parity Harness.

Compares PRODUCTION Dashbi logic (an independently re-extracted copy at
tests/fixtures/_dashbi-reference.js, built from `git show origin/main:
modules/analise-geral-grupo-secure-original-layout.html` via a SEPARATE
extraction script/algorithm/wrapper than the one that assembled
assets/js/adapters/dashbi.adapter.js) against V2's adapter across all
golden fixtures in tests/fixtures/dashbi-fixtures.json.
...
GOLDEN_PATH = os.path.join(HERE, "fixtures", "dashbi-golden-output.json")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        ref_page.goto("http://localhost:8700/portal-next-v2/tests/fixtures/_dashbi-reference-harness.html")
        if ref_errors:
        ...
        for case in fixtures:
            fixture_input = {k: case.get(k, []) for k in FIELDS}
            goldens[case["id"]] = ref_page.evaluate(
                "(fixture) => window.NX_DASHBI_REFERENCE.compute(fixture)", fixture_input
            )
        ref_page.close()
        ...
        adapter_page.goto("http://localhost:8700/portal-next-v2/tests/fixtures/_dashbi-adapter-harness.html")
        if adapter_errors:
        ...
        for case in fixtures:
            ...
            v2_result = adapter_page.evaluate(
                "(fixture) => window.NX_DASHBI_ADAPTER.compute(fixture)", fixture_input
            )
            ...
            if not match:
                print("  golden :", golden_dump[:700])
                print("  result :", v2_dump[:700])
        adapter_page.close()
        browser.close()

    total = len(results)
    passed = sum(1 for _, ok in results if ok)
    print(f"\n=== Dashbi Parity: {passed}/{total} ===")
    if passed != total:
        print("RESULT: FAIL")
        sys.exit(1)
    print("RESULT: PASS")
