/* GOLDEN REFERENCE -- independently re-extracted from origin/main for
   Gate 13/14 parity verification. Not used by the product; test-only.
   Built via extract_reference.py, a separate script/algorithm/wrapper
   from the one that assembled assets/js/adapters/coparticipado.adapter.js. */
(function () {
  'use strict';
...
// WAVE-C4B: this golden reference is defined (see file header) as an
// extraction of origin/main:modules/coparticipado.html. That source was
// fixed in Wave C.4B (bare "AWC" now resolves to the 4X4 class -- see
// MEGA_UAT_C4A_ACTIVE_NORMALIZATION_CONFLICT), checked against
// `original` (pre-cleanup) because the cleanup regex's generic `C\b`
// term eats the trailing "C" off AWC/S-AWC/SAWC, so none of the three
// ever matched against `s` -- a deeper pre-existing bug than the plain
// omission C.4A first reported. This is a manual, identical-diff mirror
// of that same fix, not a re-run of extract_reference.py -- flagged for
// a real regeneration in a later wave if strict extraction-process
// fidelity is required. Does not change the rate-lookup key: taxaKey()
// strips 4X4/4X2 identically either way (see Wave C.4B report Section 10).
...
function vendorFromRow(row){const nbs=normalizeText(getCol(row,['Vendedor','VENDEDOR','Apelido','NBS']));const nomeRaw=getCol(row,['Nome Vendedor Completo','Nome completo vendedor','Nome Vendedor','Nome','Vendedor']);const nome=normalizeText(nomeRaw);const byN=DATA.vendors.byNbs[nbs];const byName=DATA.vendors.byName[nome];return byN||byName||{nbs,nome:nome||nbs,loja:'NÃO LOCALIZADO',tipo:'',status:''}}
  ...
  window.NX_COPARTICIPADO_REFERENCE = {
