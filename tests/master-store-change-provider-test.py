...
""")
        ...
        # ---------- PM-WRITE-SAFETY-2 -- environment-authority write safety, all 4 write actions ----------
        # Network/provider-boundary proof for every real write action
        # this capability exposes -- CREATE/SET_DEPARTMENTS/SET_ACTIVE/
        # ARCHIVE all funnel through the same master_admin_manage RPC
        # name. The real server-side chain validation (23P01) is
        # entirely untouched by this gate -- a simulated call never
        # reaches the mocked route at all, so it can never trigger that
        # error either.
        sc_actions = [
            ("createStoreChange", "() => window.NX_MASTER_STORE_CHANGE_PROVIDER.createStoreChange({cpfVendedor:'11122233344', loginVendedor:'vend.um', nomeVendedor:'Vendedor Um', lojaOrigem:'LOJA CENTRO', lojaDestino:'LOJA NORTE', dataInicioOrigem:'2031-01-01', dataFimOrigem:'2031-01-31', dataInicioDestino:'2031-02-01', observacao:'', departamentoOrigem:'NOVOS', departamentoDestino:'NOVOS'})"),
            ("setDepartments", "() => window.NX_MASTER_STORE_CHANGE_PROVIDER.setDepartments('cccccccc-1000-4000-8000-000000000001', 'NOVOS', 'SEMINOVOS')"),
            ("setActive", "() => window.NX_MASTER_STORE_CHANGE_PROVIDER.setActive('cccccccc-1000-4000-8000-000000000001', false)"),
            ("archiveStoreChange", "() => window.NX_MASTER_STORE_CHANGE_PROVIDER.archiveStoreChange('cccccccc-1000-4000-8000-000000000001')"),
        ]
        env_cases = [
            ("LOCAL_DEV", False),
        ...
        for env_name, expect_real_network in env_cases:
            for action_name, action_js in sc_actions:
                ...
                page.route(WRITE_URL + "*", counting_route(env_calls, 200,
                    (lambda p: {"id": "new-id"}) if expect_real_network else (lambda p: poison)))
                mount(page)
                goto_sc(page)
                ...
                check(f"PM2-{env_name}/{action_name}c: isHomologationMode() reports {not expect_real_network}",
                      page.evaluate("window.NX_MASTER_STORE_CHANGE_PROVIDER.isHomologationMode()") == (not expect_real_network))
                page.close()
        ...
        for env_name in ["LOCAL_DEV", "AUTHORIZED_HOMOLOGATION", "AUTHORIZED_PRODUCTION", "UNKNOWN_HOST"]:
            ...
            page.route(CONV_URL + "*", json_route(200, []))
            read_calls = []
            page.route(READ_URL + "*", counting_route(read_calls, 200, lambda p: {"periods": [], "absences": [], "store_changes": [SC_RECORD]}))
            mount(page)
            goto_sc(page)
